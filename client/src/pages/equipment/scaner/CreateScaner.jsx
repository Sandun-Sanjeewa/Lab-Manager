...
import { createScaner } from "../../../services/equipment/scanerServices";


const CreateScaner = ({ onClose, onScanerCreated }) => {
    const { labs } = useLabs();
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (onScanerCreated) onScanerCreated();
            toast.success("Scaner Created Successfully!");
            onClose();

        } catch (error) {
            setError((prev) => ({
                ...prev,
                general: error.response?.data?.error || "Scaner creating failed"
            }));
        }
    };
    return (
