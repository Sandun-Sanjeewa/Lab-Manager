...
import { updateUps } from "../../../services/equipment/upsServices";



const EditUps = ({ups, onClose, onUpsUpdated }) => {
    const { labs } = useLabs();
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        try {
            await updateUps(ups._id , upsData);
            setUpsData({
                upsID: "",
                lab: "",
            ...
            setError((prev) => ({
                ...prev,
                general: error.response?.data?.error || "Ups updating failed"
            }));
        }
    };
    return (
