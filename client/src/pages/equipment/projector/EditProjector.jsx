...
import { updateProjector } from "../../../services/equipment/projectorServices";


const EditProjector = ({projector, onClose, onProjectorUpdated }) => {
    const { labs } = useLabs();
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        try {
            await updateProjector(projector._id ,projectorData);
            setProjectorData({
                projectorID: "",
                lab: "",
            ...
            if (onProjectorUpdated) onProjectorUpdated();
            ...
            setError((prev) => ({
                ...prev,
                general: error.response?.data?.error || "Projector updating failed"
            }));
        }
    };
    return (
