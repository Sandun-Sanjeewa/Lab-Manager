...
import { updatePrinter } from "../../../services/equipment/printerServices";

const EditPrinter = ({printer, onClose, onPrinterUpdated }) => {
    const { labs } = useLabs();
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        try {
            await updatePrinter(printer._id,printerData);
            setPrinterData({
                printerID: "",
                lab: "",
            ...
            setError((prev) => ({
                ...prev,
                general: error.response?.data?.error || "Printer updating failed"
            }));
        }
    };
    return (
